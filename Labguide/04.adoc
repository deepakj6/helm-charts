...
. Create a pod that defines an application container which writes the process to a file every five seconds. Create a adapter container that display the top 3 processes consuming maximum memory.

+
HINT: `ps aux --sort=pmem` . The output format may slightly vary based on the OS kernel. However, focus on VSZ (virtual memory) or %MEM column
Sample Output
...
spec:
  ...
  containers:
    ...
    - image: alpine
      name: alpine
      command: ["/bin/sh"]
      args: ["-c", "while true; do echo  $(cat /logs/output.txt |  head -4) > /logs/report.txt;  sleep 10; done"]
      volumeMounts:
        - name: shared-vol
          mountPath: /logs
------------------
